...
async function main() {
    ...
    // ===========================================================================
    const R = (id, name, role, level, locationId, skills, cap = 40) => prisma.resource.create({
        data: {
    ...
    // ===========================================================================
    const C = (id, name, industry, region, logoColor, status = 'active') => prisma.client.create({ data: { id, name, industry, region, logoColor, status } });
    ...
    // ===========================================================================
    const CAP = (id, name, description, maturity, dependsOn = []) => prisma.capability.create({ data: { id, name, description, maturity, reusePotential: 70, dependsOn } });
    ...
    // Products
    // ===========================================================================
    const P = (id, name, shortName, bucket, vision, problem, target, maturity, archStatus, aiR, delR, ownerId, tags) => prisma.product.create({
    ...
    await Promise.all(cop.map(([productId, capabilityId]) => prisma.capabilityOnProduct.create({ data: { productId, capabilityId } })));
    console.log(`  ✓ ${cop.length} capability↔product links`);
    // ===========================================================================
    // Opportunities ($9.22M pipeline)
    // ===========================================================================
    const O = (id, name, clientId, productId, value, probability, stage, ownerId, expectedCloseDate, lastInteraction, nextSteps, importance, source, description, capIds) => prisma.opportunity.create({
        data: {
    ...
    // ===========================================================================
    const PR = (id, name, code, type, clientId, productId, status, rag, ownerId, startDate, endDate, scope, charter, weeklyStatus, resourceIds) => prisma.project.create({
    ...
    // ===========================================================================
    const M = (id, name, projectId, dueDate, status, ownerId) => prisma.milestone.create({ data: { id, name, projectId, dueDate, status, ownerId } });
    ...
    // ===========================================================================
    const RK = (id, title, description, severity, likelihood, impact, status, ownerId, mitigation, opts = {}) => prisma.risk.create({
    ...
    // Decisions
    // ===========================================================================
    const D = (id, title, context, decision, rationale, status, decidedBy, alts, opts = {}) => prisma.decision.create({
    ...
    // ===========================================================================
    const AL = (resourceId, projectId, role, hoursPerWeek, weeks = 8) => prisma.resourceAllocation.create({
    ...
    // ===========================================================================
    const T = (id, title, status, priority, assigneeId, projectId, dueDate, ai = false) => prisma.task.create({
    ...
    // Action items
    // ===========================================================================
    const AI = (id, title, status, source, assigneeId, opts = {}) => prisma.actionItem.create({
        data: {
    ...
    await prisma.transcript.create({
        data: {
            ...
            uploadedBy: 'r-rahul',
            status: 'uploaded',
            rawText: `[00:00] Rahul Verma: Thanks everyone for joining. Let's start with the accuracy update.
